...
// Slot: Accept the user's email and username.
// The email must end with "@gmail.com" (case-insensitive).
...
void MainWindow::onAcceptClicked()
{
    ...
    if (!emailInput.endsWith("@gmail.com", Qt::CaseInsensitive)) {
        QMessageBox::warning(this, "Warning", "Invalid Email address. It must end with '@gmail.com'.");
        return;
    }
    ...
    if (emailChanged ^ usernameChanged) { // XOR: only one field changed
        QMessageBox::warning(this, "Warning", "Email or Username already used. Both must change together to accept a new combination.");
        return;
    }
    ...
    currentEmail = emailInput;
    currentUsername = usernameInput;
    QMessageBox::information(this, "Info", "New credentials accepted. You may now continue.");
}
